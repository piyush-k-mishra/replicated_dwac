...
from text.dwac_rnn import AttentionRnnDwac
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline', metavar='N',
                        help='Model to use [baseline|dwac_model|dwac_rnn]')
    ...
    elif args.model == 'dwac_model':
        print("Creating DWAC model using CNN")
        model = AttentionCnnDwac(args, vocab, embeddings_matrix)
    elif args.model == 'dwac_rnn':
        print("Creating DWAC model usin RNN")
        model = AttentionRnnDwac(args, vocab, embeddings_matrix)
    else:
...
def train(args, model, train_loader, dev_loader, test_loader, ref_loader, ood_loader=None):
    ...
    print("Creating output directory {:s}".format(args.output_dir + '/' + args.model))
    ...
    while not done:
        ...
        if args.model in ['dwac_rnn', 'dwac_model']:
        ...
        if dev_acc > best_dev_acc:
            ...
            epochs_without_improvement = 0
            best_epoch = epoch
            if not os.path.exists(args.output_dir + '/' + args.model):
                os.makedirs(args.output_dir + '/' + args.model)
            model_file = os.path.join(args.output_dir, args.model, 'model.best.tar')
    ...
    print("Saving")
    np.savez(os.path.join(args.output_dir, args.model, 'train.npz'),
             labels=train_labels,
             z=train_z,
    ...
    if args.model in ['dwac_rnn', 'dwac_model']:
        save_output(os.path.join(args.output_dir, args.model, 'dev.npz'),
        ...
        print("Saving")
        np.savez(os.path.join(args.output_dir, args.model, 'dev.npz'),
                 labels=dev_labels,
                 z=dev_z,
    ...
    if args.model in ['dwac_rnn', 'dwac_model']:
        save_output(os.path.join(args.output_dir, args.model, 'test.npz'),
        ...
        print("Saving")
        np.savez(os.path.join(args.output_dir, args.model, 'test.npz'),
                 labels=test_labels,
                 z=test_z,
    ...
    if ood_loader is not None:
        if args.model in ['dwac_rnn', 'dwac_model']:
            save_output(os.path.join(args.output_dir, args.model, 'ood.npz'),
            ...
            print("Saving")
            np.savez(os.path.join(args.output_dir, args.model, 'ood.npz'),
                     labels=ood_labels,
                     z=ood_z,
...
def test(args, model, test_loader, ref_loader, name='Test', return_acc=False):
    ...
    if args.model in ['dwac_rnn', 'dwac_model'] and not return_acc:
